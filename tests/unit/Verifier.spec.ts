...
describe('verifier should work properly', () => {
    describe('verify isApostille, isPublicApostille, isPrivateApostille, and isSigned ', () => {
        it('Can verify if payload is apostille', () => {
            const payload = 'fe4e545903cd39cec9bf017c5fc0f586be0a8a91dad3953295868ce47738f804446c25ce7c';

            expect(Verifier.isApostille(payload)).toEqual(true);
        });

        it('Can verify if payload is not apostille', () => {
            const payload = 'not apostille';

            expect(Verifier.isApostille(payload)).toEqual(false);
        });

        it('Can verify if payload is public apostille', () => {
            const payload = 'fe4e545903cd39cec9bf017c5fc0f586be0a8a91dad3953295868ce47738f804446c25ce7c';

            expect(Verifier.isPublicApostille(payload)).toEqual(true);
        });

        it('Can verify if payload is not public apostille', () => {
            const payload = 'fe4e545983cd39cec9bf017c5fc0f586be0a8a91dad3953295868ce47738f804446c25ce7c';

            expect(Verifier.isPublicApostille(payload)).toEqual(false);
        });

        it('Can verify if payload is private apostille', () => {
            const payload = 'fe4e545983cd39cec9bf017c5fc0f586be0a8a91dad3953295868ce47738f804446c25ce7c';

            expect(Verifier.isPrivateApostille(payload)).toEqual(true);
        });

        it('Can verify if payload is not private apostille', () => {
            const payload = 'fe4e545903cd39cec9bf017c5fc0f586be0a8a91dad3953295868ce47738f804446c25ce7c';

            expect(Verifier.isPrivateApostille(payload)).toEqual(false);
        });
    });
    ...
    describe('verify signature of public apostille', () => {
        it('Can verify signature of public apostille', () => {
            ...
            expect(Verifier.verifyPublicApostille(data, payload)).toEqual(true);
        });
        ...
        it('Return false if wrong data provided', () => {
            ...
            expect(Verifier.verifyPublicApostille(data, payload)).toEqual(false);
        });
        ...
        it('Throws error if payload is not apostille', () => {
            const data = 'I am so so so awesome as always';
            const payload = 'I am not apostille';

            expect(() => Verifier.verifyPublicApostille(data, payload)).toThrow(Errors[Errors.NOT_APOSTILLE]);
        });

        it('Throws error if payload is not public apostille', () => {
            const data = 'I am so so so awesome as always';
            const payload = 'fe4e545983cd39cec9bf017c5fc0f586be0a8a91dad3953295868ce47738f804446c25ce7c';

            expect(() => Verifier.verifyPublicApostille(data, payload)).toThrow(Errors[Errors.NOT_PUBLIC_APOSTILLE]);
        });
    ...
    describe('verify signature of private hash apostille', () => {
        ...
        it('Can verify signature of private hash apostille made from MD5', () => {
            ...
            expect(Verifier.verifyPrivateApostille(signerAccount.publicAccount, data, payload)).toEqual(true);
        });
        ...
        it('Return false if wrong data provided', () => {
            ...
            expect(Verifier.verifyPrivateApostille(signerAccount.publicAccount, data, payload)).toEqual(false);
        });
        ...
        it('Throws error if payload is not apostille', () => {
            ...
            expect(() => Verifier.verifyPrivateApostille(signerAccount.publicAccount, data, payload))
                .toThrow(Errors[Errors.NOT_APOSTILLE]);
        });

        it('Throws error if payload is not private apostille', () => {
            const data = 'I am so so so awesome as always';
            const payload = 'fe4e545903cd39cec9bf017c5fc0f586be0a8a91dad3953295868ce47738f804446c25ce7c';

            expect(() => Verifier.verifyPrivateApostille(signerAccount.publicAccount, data, payload))
                .toThrow(Errors[Errors.NOT_PRIVATE_APOSTILLE]);
        });
    });
});
